...
import (
	...
	txSenderEvm "gitlab.com/tokend/bridge/core/internal/proxy/sender/evm"
...
func NewProxyRepo(chains []data.Chain, signer signature.Signer, ipfsClient ipfs.Client) (ProxyRepo, error) {
	...
	for _, c := range chains {
		switch c.Type {
		case resources.EVM:
			txSender, err := txSenderEvm.NewTxSender(c.RpcEndpoint, signer)
			if err != nil {
				return nil, errors.Wrap(err, "failed to create evm tx sender")
			}

			proxy, err := evm.NewProxy(c.RpcEndpoint, signer, c.BridgeContract, ipfsClient, c.Confirmations, txSender)
			if err != nil {
